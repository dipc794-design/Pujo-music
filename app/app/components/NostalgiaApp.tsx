...
import { track as sendAnalyticsEvent } from "@vercel/analytics";
...
// Global window declare for YouTube API
declare global {
  interface Window {
...
// Declared strictly outside the main component so they maintain identity
// across renders and keep CSS animations (e.g. vinyl spin) smooth without restarts.
// ============================================================================
...
const VinylDisc = React.memo(function VinylDisc({
  ...
  return (
    ...
    >
      {/* Vinyl grooves background texture */}
      <div
...
// Desktop Player Component (Horizontal Glass Pill)
...
// Mobile Player Component (Stacked Card Layout)
const MobilePlayer = React.memo(function MobilePlayer({
  ...
  return (
    <div className="sm:hidden glass-panel rounded-[26px] p-4 shadow-2xl w-full flex flex-col gap-3">
      ...
      {/* Row 3: Timers left, Transport centered */}
      <div className="flex items-center justify-between">
        ...
        {/* Transport with 44px min touch targets */}
        <div className="flex items-center gap-3">
          <button
            onClick={onPrev}
            aria-label="Previous Track"
...
export default function NostalgiaApp() {
  ...
  // Helper to attach YouTube iframe to the appropriate active DOM slot
  ...
  // Handle YouTube Player API initialization
  useEffect(() => {
    let isMounted = true;
      ...
      playerRef.current = new window.YT.Player("yt-player-element", {
        ...
        events: {
          ...
          onError: (event: any) => {
            ...
            sendAnalyticsEvent("youtube_player_error", {
